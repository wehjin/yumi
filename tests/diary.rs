use echodb::{ObjectId, Ring, Say, Sayer, Arrow};
use echodb::diary::{Diary, SayPos};

#[test]
fn main() {
	let start_say = Say { sayer: Sayer::Unit, object: ObjectId::Unit, ring: Ring::Unit, arrow: Some(Arrow::Number(3)) };
	let (path, pos) = {
		let diary = Diary::temp().unwrap();
		let mut writer = diary.writer().unwrap();
		let pos = writer.write_say(&start_say).unwrap();
		assert_eq!(pos, SayPos { sayer: 0.into(), object: 1.into(), ring: 2.into(), arrow: 3.into(), end: (4 + 8).into() });
